...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        temp = self.place
        counter = 1
        while temp.entrance != hive:
            if len(temp.bees) != 0 and self.min_range < counter and counter < self.max_range:
    ...
    def action(self, colony):
        ...
        while current_place:
            if current_place.ant:
                c_ant = current_place.ant
                if c_ant not in self.doubled:
                    c_ant.damage *= 2
                    self.doubled.append(current_place.ant)
                if c_ant.is_container and c_ant.contained_ant and c_ant.contained_ant not in self.doubled:
                    self.doubled.append(c_ant.contained_ant)
                    c_ant.contained_ant.damage *= 2
